...
class TestCase(unittest.TestCase):
    ...
    def start_cassandrabeat(self,
                            cmd="../../cassandrabeat.test",
                            config="cassandrabeat.yml",
                            output="cassandrabeat.log",
                            extra_args=[],
                            debug_selectors=[]):
        """
        Starts cassandrabeat and returns the process handle. The 
    ...
    def render_config_template(self, template="cassandrabeat.yml.j2",
                               output="cassandrabeat.yml", **kargs):
        template = self.template_env.get_template(template)
        kargs["cassandrabeat"] = self
        output_str = template.render(**kargs)
        with open(os.path.join(self.working_dir, output), "wb") as f:
            f.write(output_str)

    def read_output(self, output_file="output/cassandrabeat"):
        jsons = []
    ...
    def log_contains(self, msg, logfile="cassandrabeat.log"):
        """
    ...
    def output_has(self, lines, output_file="output/cassandrabeat"):
        """
